...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      infoDetail () {
        this.webapi({
          url: '/system/api/place/queryByPrimaryKey',
          data: {
            placeInfoId: this.infoId
          }
        }).then(res => {
            const { resultCode, data, resultMessage } = res
            if (resultCode === code.CODE_SUCCESS) {
              this.ruleForm.parentId = data.parentId
              this.ruleForm.placeName = data.placeName
              this.ruleForm.placeType = data.placeType
              this.ruleForm.placeCode = data.placeCode
              const json = this.dataNative.filter(item => item.placeInfoId === data.parentId)
              this.ruleForm.parentName = json.length ? json[0].placeName : ''
            }
          })
      },
      departmentState () {
      ...
      edit () {
        ...
        this.webapi({
          url: `/system/api/place/updatePlace `,
          data: {
            ...this.ruleForm,
            placeInfoId: this.infoId
          }
