...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      delInfo () {
        if (this.tableCheck.length) {
          this.$confirm('确认删除选中数据', '提示', {
            ...
            const InfoIds = (this.tableCheck.map(item => item.monitorCameraInfoId)).join()
            this.delApi(InfoIds)
          })
        } else {
          this.$message({
      ...
      delApi (InfoIds) {
        this.webapi({
          url: '/system/api/monitorCamera/deleteByMonitorCameraInfoIdArray ',
          data: {
            monitorCameraInfoIdArray: InfoIds
          }
        }).then(res => {
