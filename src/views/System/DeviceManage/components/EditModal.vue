      ...
      >
        <section class="dialog-from-content">
          ...
          <el-form-item label="采购人电话" prop="purchaserPhone">
            <el-input v-model="ruleForm.purchaserPhone" placeholder="请输入采购人电话"/>
          </el-form-item>
          ...
          <el-form-item label="所属大系统" prop="belongSystem">
            <el-select v-model="ruleForm.belongSystem" placeholder="请选择所属大系统" @change='belongSystemChange'>
              <el-option
          ...
          <el-form-item label="所属子系统" prop="belongSubsystem">
            <!-- <el-select v-model="ruleForm.belongSubsystem" placeholder="请选择所属子系统">
              <el-option
                v-for="item in securitySystemSelect"
                :key="item.dictionaryDataInfoId"
                :label="item.dictionaryValue"
                :value="item.dictionaryDataInfoId"
              />
            </el-select> -->
            <!-- <el-input :value='ruleForm.belongSubsystem'/> -->
            <!-- <span>
              ------数据---- {{ ruleForm.belongSubsystem }}<br/>
              ------数据---- {{ JSON.stringify(ruleForm) }}<br/>
              ----------安防---- {{ JSON.stringify(securitySystemSelect) }}<br/>
              ---------消防---- {{ JSON.stringify(fireSystemSelect) }}<br/>
            </span> -->
            <el-select
              v-model="ruleForm.belongSubsystem"
              placeholder="请选择所属子系统"
              v-if='ruleForm.belongSystem==="7864386838056689664"'
            >
              <el-option
                v-for="item in fireSystemSelect"
                :key="item.dictionaryDataInfoId"
                :label="item.dictionaryValue"
                :value="item.dictionaryDataInfoId"
              />
            ...
            <el-select
              v-model="ruleForm.belongSubsystem"
              placeholder="请选择所属子系统"
              v-else-if='ruleForm.belongSystem==="7864386838056689665"'
            >
              <el-option
                v-for="item in securitySystemSelect"
                :key="item.dictionaryDataInfoId"
  ...
  export default {
    ...
    computed: {
      ...mapState({
        ...
        fireSystemSelect: state => state.fireSystem,
        securitySystemSelect: state => state.securitySystem
      }),
      pickerOptionsBegin () {
        return {
          disabledDate: (time) => {
    ...
    methods: {
      ...
      infoDetail () {
        this.webapi({
          url: '/system/api/device/getDeviceInfoDetail',
          data: {
          ...
          if (resultCode === code.CODE_SUCCESS) {
            ...
            this.ruleForm.palceParentNames = data.buildingName + ',' + data.floorName + ',' + data.roomName
            this.ruleForm.palceParentIds = data.affiliatedBuilding + ',' + data.affiliatedFloor + ',' + data.affiliatedRoom
          }
        })
      },
      belongSystemChange (e) {
        this.ruleForm.belongSubsystem = ''
      },
      addMenu () {
        ...
        this.webapi({
          url: '/system/api/device/updateDeviceInfo',
          data: {
            ...this.ruleForm,
            deviceInfoId: this.infoId
          }
    ...
    watch: {
      ...
      'ruleForm.belongSystem': function (val) {
        
      },
      'ruleForm.belongSubsystem': function (val) {
        
      },
      'securitySystemSelect': function (val) {
