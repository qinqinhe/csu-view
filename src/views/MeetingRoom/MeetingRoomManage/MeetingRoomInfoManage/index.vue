    ...
    <section class="main-data">
      ...
      <section class="main-from">
        <el-form ref="ruleForm" label-width="95px" :model="ruleForm">
          ...
          <el-form-item label="管理员名字" prop="meetingRoomAdminUserName">
            <el-input placeholder="管理员名字" size="medium" v-model="ruleForm.meetingRoomAdminUserName"/>
          </el-form-item>
          <el-form-item label="会议室状态" prop="meetingRoomStatus">
            <el-select
              v-model="ruleForm.meetingRoomStatus"
              size="medium"
              clearable
              placeholder="会议室状态"
            >
              <el-option
                v-for="item in meetingRoomStatusSelect"
                :key="item.value"
                :label="item.label"
                :value="item.value"
              />
      ...
      <section class="main-table">
        <section class="main-table-con">
          ...
          >
            ...
            <el-table-column prop="meetingRoomName" align="center" label="会议室名称"/>
            <el-table-column prop="meetingRoomCode" label="会议室编码" align="center"/>
            <el-table-column prop="capacityNum" align="center" label="可容纳人数"/>
            <el-table-column prop="meetingRoomAdminUserName" align="center" label="管理员"/>
            <el-table-column prop="meetingRoomAdminPhone" align="center" label="管理员联系电话"/>
            <el-table-column prop="meetingRoomStatus" align="center" label="状态"/>
            <el-table-column label="会议室位置" align="center" prop="affiliated" width="250">
              <!-- <el-table-column prop="affiliatedBuilding" label="楼" align="center" />
              <el-table-column prop="affiliatedFloor" label="层" align="center" />
              <el-table-column prop="affiliatedRoom" label="房间" align="center" /> -->
            </el-table-column>
            <el-table-column label="操作" align="center" width="160">
              <template slot-scope="scope">
                <el-button size="mini" type="primary" @click="edit(scope.row.meetingRoomInfoId)">编辑</el-button>
                <el-button size="mini" type="danger" @click="del(scope.row.meetingRoomInfoId)">删除</el-button>
              </template>
            </el-table-column>
          </el-table>
        </section>
  ...
  export default {
    ...
    async mounted () {
      ...
      await this.webapi({
        ...
        data: {
          dictionaryTypeCode: dictionary.meeting.code
        }
      }).then(res => {
        const { resultCode, data, resultMessage } = res
        if (resultCode === code.CODE_SUCCESS) {
    ...
    methods: {
      ...
      inquiry () {
        ...
        this.webapi({
          url: '/meeting/api/meetingRoom/queryMeetingRoomByPage',
          data: {
            ...data
          }
        }).then(res => {
          ...
          if (resultCode === code.CODE_SUCCESS) {
            ...
            this.tableData = rows.map(item => {
              item.affiliated = `${item.affiliatedBuilding},${item.affiliatedFloor},${item.affiliatedRoom}`
              item.meetingRoomStatus = item.meetingRoomStatus === '0' ? '禁用' : '启用'
              return item
            })
      ...
      delApi (InfoId) {
        this.webapi({
          url: '/meeting/api/meetingRoom/deleteMeetingRoom',
          data: {
            meetingRoomInfoId: InfoId
          }
        }).then(res => {
          const { resultCode, data, resultMessage } = res
          if (resultCode === code.CODE_SUCCESS) {
