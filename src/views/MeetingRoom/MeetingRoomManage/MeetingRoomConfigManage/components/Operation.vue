		...
		methods: {
			...
			delInfo() {
				if(this.tableCheck.length) {
					const InfoIds = this.tableCheck.map(item => item.timeArrangeConfigId).join(",");					
			...
			delApi(InfoIds) {
				this.webapi({ //管理员批量删除陈列室信息 
					url: "/meeting/api/timeArrangeConfig/deleteByTimeArrangeConfigIdArray",
					data: {
						timeArrangeConfigIdArray: InfoIds
					}
				}).then(res => {
					const {
						resultCode,
