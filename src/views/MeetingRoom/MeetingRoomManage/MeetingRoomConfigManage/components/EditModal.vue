	...
	<div class="modal-dialog">
        ...
        <section class="dialog-from-content">
          ...
          <el-form-item label="会议室" prop="meetingRoom">
            <el-checkbox-group v-model="ruleForm.meetingRoom">
              <el-checkbox v-for="item in [...meetingRoom, ...timeArrangeRoomVo]" :label="item.meetingRoomInfoId" :key="item.meetingRoomInfoId"  v-model="item.meetingRoomInfoId">{{ item.meetingRoomName }}</el-checkbox>
            </el-checkbox-group>
          </el-form-item>
          <section class="table-add">
            ...
            <el-table
              :data="ruleForm.timeArrangeAddVoList"
              style="width: 100%"
              >
              ...
              <el-table-column label="排序" width="90" align='center'>
                <template slot-scope="scope">
                  <el-form-item
                    label-width='0'
                    :prop='"timeArrangeAddVoList." + scope.$index + ".timeSort"' 
              ...
              <el-table-column label="时间段" align='center'>
                <template slot-scope="scope">
                  <el-form-item
                    label-width='0'
                    :prop='"timeArrangeAddVoList." + scope.$index + ".time"' 
                    :rules='rules.time'
                    >
                    <el-time-picker
                      is-range
	...
	export default {
		...
		computed: {
			...mapState({
				...
				meetingRoom: state => state.meetingRoomConfigManage.meetingRoom
			})
		},
		methods: {
			...mapMutations([
			...
			addTtemplate () {
				this.ruleForm.timeArrangeAddVoList.push({
							startTime: '',
							endTime: '',
							timeSort: '',
							time: ''
			...
			delTtemplate (index) {
				this.ruleForm.timeArrangeAddVoList.splice(index, 1)
			},
			
			infoDetail() {
				this.webapi({
					url: `/meeting/api/timeArrangeConfig/queryTimeArrangeConfigDetail`,
					data: {
					...
					if(resultCode === code.CODE_SUCCESS) {
						...
						ruleForm.timeArrangeAddVoList = data.timeArrangeQueryVoList.map(item => {
							item.time = [item.startTime, item.endTime]
							return item
						})
						ruleForm.meetingRoom = data.timeArrangeRoomQueryVoList.map(item => {
							return item.roomId
						})
						this.ruleForm = ruleForm;
						let timeArrangeRoomVo = data.timeArrangeRoomQueryVoList.map(item => {
							return {
								meetingRoomInfoId: item.roomId,
								meetingRoomName: item.roomName
							}
						})
						this.timeArrangeRoomVo = timeArrangeRoomVo
					}
				});
			},
			edit () {
				let ruleForm = JSON.parse(JSON.stringify(this.ruleForm))
				for (let i = 0;i<ruleForm.timeArrangeAddVoList.length;i++) {
					const item = ruleForm.timeArrangeAddVoList[i]
					ruleForm[`timeArrangeAddVoList[${i}].timeSort`] = item.timeSort
					ruleForm[`timeArrangeAddVoList[${i}].startTime`] = item.time[0] + ':00'
					ruleForm[`timeArrangeAddVoList[${i}].endTime`] = item.time[1] + ':00'
				}
				delete ruleForm.timeArrangeAddVoList
      	ruleForm.roomIdArray  = ruleForm.meetingRoom.join(',')
				delete ruleForm.meetingRoom
				this.webapi({
					url: "/meeting/api/timeArrangeConfig/updateTimeArrangeConfig",
					data: {
						...ruleForm,
						timeArrangeConfigId: this.infoId
					}
			...
			submitForm(formName) {
				this.$refs[formName].validate(valid => {        
					if (this.ruleForm.timeArrangeAddVoList.length) {
						if (valid) {
							this.edit();
						}
					} else {
