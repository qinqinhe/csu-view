        ...
        <section class="dialog-from-content">
          ...
          <el-form-item label="会议室" prop="meetingRoom">
            <el-checkbox-group v-model="ruleForm.meetingRoom">
              <el-checkbox v-for="item in meetingRoom" :label="item.meetingRoomInfoId" :key="item.meetingRoomInfoId"  v-model="item.meetingRoomInfoId">{{ item.meetingRoomName }}</el-checkbox>
            </el-checkbox-group>
          </el-form-item>
...
export default {
  ...
  methods: {
    ...
    add () {
      ...
      this.webapi({
        url: "/meeting/api/timeArrangeConfig/addTimeArrangeConfig ",
        data: {
          ...ruleForm
        }
      }).then(res => {
