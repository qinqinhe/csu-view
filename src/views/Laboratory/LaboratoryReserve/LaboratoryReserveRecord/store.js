...
export default {
  ...
  mutations: {
    ...
    laboratoryTypeSet (state, array) {
      state.laboratoryType = array
    },
  ...
  actions: {
    async inquiry ({ commit, state }) {
      const { resultCode, data } = await webapi({
        url: '/laboratory/api/reserveInfo/queryReserveInfoLaboratory ',
        data: {
          ...state.pagination,
    ...
    async laboratoryTypeSet ({ commit, state }) {
      ...
      if (resultCode === code.CODE_SUCCESS) {
        const json = {}
        data.forEach(item => {
          json[item.dictionaryTypeInfoId] = item.dictionaryValue
        })
    ...
    async editDetail ({ commit, state }, infoId) {
      const { resultCode, data } = await webapi({
        url: '/laboratory/api/reserveInfo/queryReserveInfoLabDetails',
        data: {
      ...
      if (resultCode === code.CODE_SUCCESS) {
        ...
        commit('editRuleFormUpdate', {
          ...
          reserveObjectName: data.laboratoryName,
          reserveType: '1',
    ...
    async editsubmitForm ({ commit, state, dispatch }) {
      ...
      const { resultCode, data } = await webapi({
        url: '/laboratory/api/reserveInfo/updateReserveInfoLaboratory ',
        data: {
    ...
    async delDateRequest ({ state, dispatch }) {
      const { resultCode } = await webapi({
        url: '/laboratory/api/reserveInfo/deleteReserveInfoLaboratory',
        data: {
