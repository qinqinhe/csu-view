  ...
  actions: {
    async inquiry ({ commit, state }) {
      const { resultCode, data } = await webapi({
        url: '/laboratory/api/laboratoryAudit/queryLaboratoryAudit ',
        data: {
          ...state.pagination,
    ...
    async editDetail ({ commit, state }, infoId) {
      const { resultCode, data } = await webapi({
        url: '/laboratory/api/laboratoryAudit/queryReserveLaboratoryDetail ',
        data: {
    ...
    async editsubmitForm ({ commit, state, dispatch }) {
      ...
      const { resultCode, data } = await webapi({
        url: '/laboratory/api/laboratoryAudit/addLaboratoryAudit ',
