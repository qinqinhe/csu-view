    ...
    <section class="main-data">
      ...
      <section class="main-from">
        <el-form ref="ruleForm" label-width="95px" :model="ruleForm">
          ...
          <el-form-item label="管理员名字" prop="laboratoryAdminName">
            <el-input placeholder="管理员名字" size="medium" v-model="ruleForm.laboratoryAdminName"/>
          </el-form-item>
          <el-form-item label="实验室状态" prop="laboratoryStatus">
            <el-select v-model="ruleForm.laboratoryStatus" size="medium" clearable placeholder="实验室状态">
              <el-option
                v-for="item in laboratoryStatusSelect"
                :key="item.value"
                :label="item.label"
                :value="item.value"
              />
      ...
      <section class="main-table">
        <section class="main-table-con">
          <el-table :data="tableData" height="100%" border tooltip-effect="dark" @selection-change="tableCheckChange">
            ...
            <el-table-column prop="laboratoryName" align="center" label="实验室名称"/>
            <el-table-column prop="capacityNum" align="center" label="可容纳人数"/>
            <el-table-column prop="laboratoryAdminName" align="center" label="管理员"/>
            <el-table-column prop="laboratoryAdminPhone" align="center" label="管理员联系电话"/>
            <el-table-column prop="laboratoryStatus" align="center" label="状态"/>
            <el-table-column prop="laboratoryTypeValue" label="类型名称" align="center" width="100"/>
            <el-table-column label="实验室位置" align="center" prop="address" width="180"/>
            <el-table-column label="操作" align="center" width="160">
              <template slot-scope="scope">
                <el-button size="mini" type='primary' @click="edit(scope.row.laboratoryInfoId)">编辑</el-button>
                <el-button size="mini" type="danger" @click="del(scope.row.laboratoryInfoId)">删除</el-button>
              </template>
            </el-table-column>
          </el-table>
        </section>
        <el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange"
                       :current-page.sync="pagination.pageCurrent" :page-sizes="[1, 10, 50, 100]"
                       :page-size="pagination.pageRows" layout="total, sizes, prev, pager, next, jumper"
  ...
  export default {
    ...
    async mounted () {
      ...
      await this.webapi({
        ...
        data: {
          dictionaryTypeCode: dictionary.laboratory.code
        }
      }).then(res => {
        const {
          resultCode,
    ...
    methods: {
      ...
      inquiry () {
        ...
        this.webapi({
          url: '/laboratory/api/laboratory/queryLaboratoryByPage',
          data: {
            ...data
          }
        }).then(res => {
      ...
      delApi (InfoId) {
        this.webapi({
          url: '/laboratory/api/laboratory/delLaboratory  ',
          data: {
            laboratoryInfoId: InfoId
          }
        }).then(res => {
          const {
            resultCode,
