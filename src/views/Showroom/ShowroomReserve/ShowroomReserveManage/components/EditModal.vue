        ...
        <section class="dialog-from-content">
          ...
          <el-form-item label="预约人员" prop="reserveUser">
            <!-- <el-checkbox-group v-model="ruleForm.reserveUser">
              <el-checkbox :label="item.departmentName" v-for='(item, index) in students' :key='index'></el-checkbox>
            </el-checkbox-group> -->
            <!-- <el-tree
               :data="students"
              show-checkbox
              node-key="id"
              @check='(data, checked) => studentsCheck({data, checked})'
...
<script>
  ...
  export default {
    ...
    computed: {
      ...mapState({
        visible: state => state.showroomReserveManage.editDate.visible,
        ruleForm: state => state.showroomReserveManage.editDate.ruleForm,
        rules: state => state.showroomReserveManage.editDate.rules,
        
        students: state => state.showroomReserveManage.editDate.students
      })
    },
    methods: {
