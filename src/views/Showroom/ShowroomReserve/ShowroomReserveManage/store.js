  ...
  actions: {
    ...
    async conditionsPeriodResponse ({ commit }) {
      const { resultCode, data } = await webapi({
        url: '/showroom/api/timeArrangeConfig/getShowroomConfigList'
      })
    ...
    async inquiry ({ commit, state }) {
      const { showroomName, time, date, coordinateArrayVo } = state.conditionsForm.ruleForm
      ...
      const { resultCode, data } = await webapi({
        url: '/showroom/api/showroom/getShowroomListByUser',
        data: {
          ...state.pagination,
          ...ruleForm
        }
    ...
    async editsubmitForm ({ commit, state, dispatch }) {
      ...
      const { resultCode, data } = await webapi({
        url: '/showroom/api/reserveInfo/addReserveShowroom',
        data: {
          ...ruleForm
        }
      })
