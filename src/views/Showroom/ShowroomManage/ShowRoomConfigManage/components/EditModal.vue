        ...
        <section class="dialog-from-content">
          ...
          <el-form-item label="陈列室" prop="showRoom">
            <el-checkbox-group v-model="ruleForm.showRoom">
              <el-checkbox v-for="item in [...showRoom, ...timeArrangeRoomVo]" :label="item.showroomInfoId" :key="item.showroomInfoId"  v-model="item.showroomInfoId">{{ item.showroomName }}</el-checkbox>
            </el-checkbox-group>
          </el-form-item>
...
export default {
	...
	methods: {
		...
		infoDetail() {
			this.webapi({
				url: `/showroom/api/timeArrangeConfig/getTimeArrangeConfigDetail`,
				data: {
				...
				if(resultCode === code.CODE_SUCCESS) {
					...
					ruleForm.timeArrangeAddVo = data.timeArrangeVo.map(item => {
						item.time = [item.startTime.substr(11, 5), item.endTime.substr(11, 5)]
						return item
					})
					ruleForm.showRoom = data.timeArrangeRoomVo.map(item => {
						return item.roomId
					})
					this.ruleForm = ruleForm;
					let timeArrangeRoomVo = data.timeArrangeRoomVo.map(item => {
						return {
							showroomInfoId: item.roomId,
							showroomName: item.roomName
						}
					})
		...
		edit () {
			...
			this.webapi({
				url: "/showroom/api/timeArrangeConfig/updateTimeArrangeConfig",
				data: {
					...ruleForm,
					timeArrangeConfigId: this.infoId
				}
