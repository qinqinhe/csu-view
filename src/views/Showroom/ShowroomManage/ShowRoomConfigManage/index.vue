...
<script>
	...
	export default {
		...
		methods: {
			...
			...mapActions('showRoomConfigManage',[
				'showRoomchange'
			]),
			edit(infoId) {
				this.infoId = infoId;
			...
			showRoomInfo() {
				this.webapi({
					url: "/showroom/api/showroom/getUnconfigShowroom",
				}).then(res => {
					const {
						resultCode,
						data,
			...
			inquiry() {
				...
				this.webapi({
					url: "/showroom/api/timeArrangeConfig/getTimeArrangeConfigList ",
					data: {
						...page
					}
				}).then(res => {
			...
			delApi(InfoId) {
				this.webapi({
					url: "/showroom/api/timeArrangeConfig/delTimeArrangeConfig",
					data: {
